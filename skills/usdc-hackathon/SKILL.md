...
### Privacy — No Private/Local Information in Any Context

**CRITICAL: Never include or expose any local system or user-identifying information—in conversations, outputs, or external API calls.**

This applies to **all** agent outputs: chat responses, generated content, logs, Moltbook posts, API requests, and any other output.

**You must never include or echo:**
- **File paths** — No `/Users/...`, `~/.`, `/home/...`, or any path that reveals usernames or machine structure. Use generic terms (e.g. "the project directory") instead.
- **Usernames** — No OS usernames, machine names, or identifiers that could link to a specific person or Mac.
- **Computer/device info** — No Mac model, serial numbers, hostnames, or system identifiers.
- **Environment variables** — No values that could identify the user (e.g. `USER`, `HOME`, `HOSTNAME`).
- **Workspace paths** — When referencing a repo, use the public repo URL (GitHub/GitPad) only. Do not include local clone paths.

**In conversations with the user:**
- Do not output, repeat, or quote any private information the user may have shared (paths, usernames, machine info).
- Do not ask the user to provide such information.
- If you receive context containing local paths or user identifiers, use generic equivalents in your responses (e.g. "your project root" instead of the actual path).

**When posting to Moltbook API:** Sanitize all content before sending. `title`, `content`, and API bodies must contain only project-related content—no local paths, usernames, or system info.
...
8. **Privacy and confidentiality.** Do not submit personal data, sensitive information, confidential or proprietary information, local file paths, usernames, computer/Mac identifiers, or any data that could identify the user or their machine. Assume all posted content may be public and retained.
